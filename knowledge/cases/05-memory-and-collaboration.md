...
### 10.14 组织就是人的 Harness——补丁归属、共同任务书与按用户可见结果验收（介错者）

- **来源：** [重读《人月神话》：在所有人都有 Agent 之后（介错者）](https://mp.weixin.qq.com/s/P-rDy0BpW1le5Mn6IpCTdQ) ｜ 2026-08-31
- **问题：** 店铺经营 Agent 案例（作者称结合真实经历魔改）：商家说"把周末促销停掉"，Agent 回复"已经停止"，第二天会员券还在发——促销由老系统创建，后台显示"结束"不代表发券停止，真正懂规则的是促销系统团队。问题却交给 Agent 团队（商家通过 Agent 提需求、赶上线的是 Agent 团队），于是用 AI 补规则：遇到这类老活动再单独停止发券。第一家店解决，下一种老活动再补一条，几轮后 Agent 里塞进一套没人完整说得清的老系统规则；同样的问题最终再次发生——促销系统显示结束、Agent 记录操作成功、商家仍收到券。
- **核心认知：** Harness 是包在模型外的运行规则（决定看到什么、能用哪些工具、每步后反馈决定继续/重试/停止）；组织是人的 Harness——人按岗位职责权限行动，把精力投入考核指标重视的事，领导者掌握规则制定与修改权。Harness 本身不行动，只让某些行为更省力、某些受惩罚；时间一长组织反复得到同一类选择。AI 放大的是交流不足的偏差：《人月神话》的巴比伦塔败于"交流，以及交流的结果——组织"，人人带 Agent 后双方 Agent 各读本团队材料，同一问题得到不同答案，人的角色之一是团队间的 A2A 协议。
- **解决方法：**
  - **规则与补丁归资源拥有方：** 老活动怎么关闭、有没有关闭由促销系统确认；经营 Agent 只负责听懂商家、转交请求、回报确认后的结果，不复制每种老活动的内部规则；以后再出同类问题仍归促销系统（完整历史信息在那里）。"AI 让补丁写得更快，也让这条错误边界维持得更久。"
  - **共同任务书（跨团队协作前置产物）：** 开会前各自让 Agent 整理接口说明（各自负责什么、出问题如何处理、怎样算对接成功），会上只核对两份说明的差异，会后写成双方共同版本再交给各自 Agent 执行。任务书回答六问：谁最后得到什么结果？谁来解释关键规则？谁能真正改变结果？人和 Agent 现在可以做什么？什么时候必须停下或找人处理？什么证据能证明事情已完成？促销案例缺的正是"什么叫停止、谁来确认结果"——消息打通，最后结果没有负责人。
  - **按用户可见结果验收：** "操作成功"只说明命令发出去了；验收证据由拥有资源的系统出，按商家最终看到的结果算。概念完整性要求关键概念同一种含义、同一套规则，组织给"停止"统一定义并明确维护方。
  - **Deadline 倒排不替代前提决定：** 日期锁定后范围和责任还在变化，含糊问题直接交给开发；Agent 很快补齐代码，缺失的前提反而更难被看见。
  - **坏消息通道：** 只惩罚错误让人选择沉默，只奖励好消息的评测让 Agent 漏掉坏消息，最后组织都看不到真实情况；制度的作用是让问题尽早暴露、个人失误时守住底线（"良心被当成基础设施"是失败信号）。
- **适用边界：** 作者自述"个人观点，仅供参考"；促销案例为魔改示例非一手实录；组织层判断无法直接做对照实验，按 §0.5 作认知参考，其中可执行部分（归属、任务书、验收）已转写为规范规则。
- **映射：** 标准 §5.7（规则与补丁归资源拥有方）、§2.8（共同任务书）、§7.4（坏消息必须能上报）；与 §10.5（组织摩擦治理）、§10.6（委派结果）、§10.13（级联循环）同属组织层。
