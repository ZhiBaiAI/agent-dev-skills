...
- **SHOULD** 信号设计保证坏消息能上报：只惩罚错误会让人选择沉默，只奖励好消息的评测会让 Agent 漏掉坏消息，两条路都通向组织看不到真实情况；考核指标与评测信号要为暴露问题留出安全通道。
...
- **MUST** 验证结果以结构化证据记录：跑了什么命令、退出码、哪些用例失败、是否满足停止条件；Agent 对验证结果的自述摘要（如"测试通过大部分，只剩少量无关问题"）不构成通过证据，直接判失败。
